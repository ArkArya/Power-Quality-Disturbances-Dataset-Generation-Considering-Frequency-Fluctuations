% function xs = c20_sag_with_harmonics_with_flicker_freq( )
            ...
            for alpha5=0.05:0.05:0.15
                for lamda=0.05:0.01:0.1
                    for fn=8:4:25
                        ...
                        x=@(t) sin(2*pi*F*t) + (1 + lamda* sin(2*pi*fn*t)) .* (alpha1*sin(1*2*pi*F*t-vn) + alpha3*sin(3*2*pi*F*t-vn) + alpha5*sin(5*2*pi*F*t-vn)) .* ( (-1)* alpha *(heaviside(t-t1)-heaviside(t-t2))); %% Generated Signal
                        flag = flag + 1;
                        count = count + 1;
...
% csvwrite('dataset20_20db.csv', xs_20);
% csvwrite("dataset20_freq.csv",xs);
